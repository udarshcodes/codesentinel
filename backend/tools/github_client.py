...
def prepare_repo_for_push(repo_url: str, local_path: str, token: str) -> dict:
    ...
    subprocess.run(
        ["git", "checkout", "-b", branch_name], cwd=local_path, check=True, timeout=30
    )
    subprocess.run(
...
def commit_and_push(
    ...
    for f in files:
        if os.path.exists(os.path.join(local_path, f)):
            try:
                subprocess.run(
                    ["git", "add", f], cwd=local_path, check=True, timeout=30
                )
    ...
    # Embed credentials in the push URL to avoid leaking the token in process arguments (visible in `ps aux`)
    if "github.com/" in push_repo_url:
        authenticated_url = push_repo_url.replace(
            "https://github.com/", f"https://oauth2:{token}@github.com/"
        )
    else:
        authenticated_url = push_repo_url

    subprocess.run(
        ["git", "remote", "add", "auth_origin", authenticated_url],
        cwd=local_path,
        capture_output=True,
        timeout=10,
    )
    subprocess.run(
        ["git", "remote", "set-url", "auth_origin", authenticated_url],
        cwd=local_path,
        capture_output=True,
        timeout=10,
    )
