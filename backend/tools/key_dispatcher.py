        ...
        if not available:
            if GROQ_EMERGENCY_KEY:
                if not _state["emergency_active"]:
                    ...
                    print(
                        "[KeyDispatcher] All primary keys exhausted. Activating emergency key 6."
                    )
                    _fire_alert()
                return GROQ_EMERGENCY_KEY, -1
            raise RuntimeError(
                "[KeyDispatcher] All API keys exhausted including emergency key."
            )
...
def record_usage(key_index: int, tokens_used: int):
    ...
    with _lock:
        ...
        if _state["usage"][key_index] >= GROQ_TOKENS_PER_KEY_PER_DAY:
            ...
            print(
                f"[KeyDispatcher] Key index {key_index} hit daily budget. Removing from pool."
            )
        ...
        return {
            ...
            "primary_keys": {
                str(i): {
                    ...
                    "percent_used": round(
                        primary_usage.get(i, 0) / GROQ_TOKENS_PER_KEY_PER_DAY * 100, 1
                    ),
            ...
            "emergency_key": {
                ...
                "budget": GROQ_TOKENS_PER_KEY_PER_DAY,
                "percent_used": round(
                    _state["emergency_usage"] / GROQ_TOKENS_PER_KEY_PER_DAY * 100, 1
                ),
            },
            "summary": {
                "total_tokens_used": sum(primary_usage.values())
                + _state["emergency_usage"],
